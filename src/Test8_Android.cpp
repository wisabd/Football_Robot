...
int main() {
	...
	CInternalState emotion(happyTH, angryTH, fearTH, sadTH);						// Create internal state
	...
	int jointInitAng[jointNum] = {0, 0,-40,40};
	//2. Set IDs of motors
	...
	rit::server myServer(io, 4567);
	boost::thread phoneThread(boost::bind(&boost::asio::io_service::run, &io));
	...
	while (exitFlag == false) {
		...
		BehSelFromAndroid(sound, myServer, actuator);
		//3. Clear perception
		perception.Clear();
